...
public class SecurityConfig extends WebSecurityConfigurerAdapter { //Extendendo a classe de configurção, para que póssamos fazer nossa próprias configurações de segurança
    ...
    protected void configure(HttpSecurity http) throws Exception{
        ...
        http.csrf().disable().authorizeRequests()//Habilitanto os methodos além do GET
                .antMatchers("/api/admin/**", "/swagger-ui.html").hasRole("ADMIN") //Estamos informando que determinada URL só pode ser acessada por usuários ADMIN
                .antMatchers("/api/auth/**").authenticated() //Todas as requisiçoes que precisam ser autenticadas
                .anyRequest().permitAll() //Caminhos que não precisam de autenticação (no caso o restante exceto o listado acima)
                /*.and()
                ...
                */
                .and()
                .exceptionHandling().accessDeniedPage("/403") //Informando qual requisição chamar caso tenhamos o erro de não autorizado
                .and()
                .httpBasic() //Informando que o método de autenticação será o httpBasic (usuário e senha codificado em 64bit)
                .and()
    ...
    public void configure(WebSecurity web) {
        web.ignoring().antMatchers("/css/**", "/icons/**"); //Aqui informamos pastas e arquivos que serão ignorados pelo security
    }
}